...
public class UtilizationModelPlanetLabInMemory implements UtilizationModel {
	...
	public UtilizationModelPlanetLabInMemory(String inputPath, double schedulingInterval)
			...
			IOException {
		data = new double[289]; // No of entry in the files
		setSchedulingInterval(schedulingInterval); // 300 in the constants - Shashi
		BufferedReader input = new BufferedReader(new FileReader(inputPath));
		int n = data.length;
		for (int i = 0; i < n - 1; i++) {
			data[i] = Integer.valueOf(input.readLine()) / 100.0;  // to make value in the range from 0-1
		}
		data[n - 1] = data[n - 2];
		input.close();
	}
	...
	public UtilizationModelPlanetLabInMemory(String inputPath, double schedulingInterval, int dataSamples)
		...
		data = new double[dataSamples]; // number of data sample entry it takes. Indicating how many samples you want
		// should be less than or equal to number of entry in data file
		BufferedReader input = new BufferedReader(new FileReader(inputPath));
		int n = data.length;
		for (int i = 0; i < n - 1; i++) {
			data[i] = Integer.valueOf(input.readLine()) / 100.0;

			Log.printDebugMessages("Log Debug CPU Util Data: PlanetLab Cloudlet- " + i + " = " + data[i] );
			System.out.println("System print- CPU Util Data: PlanetLab Cloudlet- " + i + " = " + data[i]);
			Log.printLine(" Log pirnt CPU Util Data: PlanetLab Cloudlet- " + i + " = " + data[i]);
		}
		data[n - 1] = data[n - 2];
		input.close();
	}

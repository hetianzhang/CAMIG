...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	/** The destination candidate matrix*/
	private List<Map<Integer, List<? extends Host>>> dstCandidateMatrix = new ArrayList<>();


	Map<Integer, Vm> idtoVmMap = new HashMap<>();
	private final boolean singleVMViewSelection = false;
	private final boolean hostHitPolicy = true;
	/**
...
//sas
	public List<Map<Integer, List<? extends Host>>> getAllocationMatrix(){
		return this.dstCandidateMatrix;
	}

	public Map<Host, Vm> getOverUtilizedHostVmMap(){
		Map<Host, Vm> overUtilizedHost = new HashMap<>();
		//over-utilized migrations
		List<PowerHostUtilizationHistory> overUtilizedHosts = getOverUtilizedHosts();
		List<? extends Vm> vmsToMigrate = getVmsToMigrateFromHosts(overUtilizedHosts);
		this.dstCandidateMatrix = getVmPlacementCandidateMatrix(vmsToMigrate, new HashSet<Host>(
				overUtilizedHosts));
		for(Vm vm : vmsToMigrate){
			PowerHost oldHost = (PowerHost) vm.getHost();
			overUtilizedHost.put(oldHost, vm);
		}
		return overUtilizedHost;
	}

	public Map<Host, List<? extends Vm>> getUnderUtilizedHostVmListMap(List<PowerHostUtilizationHistory> overUtilizedHosts){
		Map<Host, List<? extends Vm>> underUtilizedHostVmListMap = new HashMap<>();
		...
		while(true) {
			...
			underUtilizedHostVmListMap.put(underUtilizedHost, vmsToMigrateFromUnderUtilizedHost);
		}

		return underUtilizedHostVmListMap;
	}


	Map<Host, Vm> overUtilizedHostVMMap = new HashMap<>();
	Map<Host, List<? extends Vm>> underUtilizedHostVMsMap = new HashMap<>();

	/**
	 *
	 * @param overUtilizedHosts fixed
	 * @param underUtilizedHosts fixed
	 * @param overUtilizedVmsToMigrate remaining over utilized vms
	 * @param excludedOverUtilizedHosts already selected
	 * @param excludedUnderUtilizedHosts already selected
	 */
	public void optimizeVmAllocationCandidates(List<Map<String, Object>> newVmPlacement, List<PowerHost> overUtilizedHosts, List<PowerHost> underUtilizedHosts, List<? extends Vm> overUtilizedVmsToMigrate, List<PowerHost> excludedOverUtilizedHosts, List<PowerHost> excludedUnderUtilizedHosts){
		this.dstCandidateMatrix.clear();
		underUtilizedHostVMsMap.clear();
		overUtilizedHosts.addAll(extractHostListFromMigrationMap(newVmPlacement));
		this.dstCandidateMatrix = getVmPlacementCandidateMatrix(overUtilizedVmsToMigrate, new HashSet<Host>(
				overUtilizedHosts));
		List<PowerHost> switchedOffHosts = getSwitchedOffHosts();
		...
		excludedHostsForFindingNewVmPlacement.addAll(overUtilizedHosts);
		excludedHostsForFindingNewVmPlacement.addAll(switchedOffHosts);
		excludedHostsForFindingNewVmPlacement.addAll(excludedUnderUtilizedHosts);
		excludedHostsForFindingNewVmPlacement.addAll(excludedOverUtilizedHosts);

		underUtilizedHosts.removeAll(excludedUnderUtilizedHosts);
		underUtilizedHosts.removeAll(overUtilizedHosts);
		underUtilizedHosts.removeAll(switchedOffHosts);
		for(PowerHost underUtilizedHost:underUtilizedHosts){
			List<? extends Vm> vmsToMigrateFromUnderUtilizedHost = getVmsToMigrateFromUnderUtilizedHost(underUtilizedHost);
			if (vmsToMigrateFromUnderUtilizedHost.isEmpty()) {
				continue;
			}
			excludedHostsForFindingNewVmPlacement.add(underUtilizedHost);
			this.dstCandidateMatrix.addAll(getVmPlacementCandidateMatrix(vmsToMigrateFromUnderUtilizedHost,
					excludedHostsForFindingNewVmPlacement));
			this.underUtilizedHostVMsMap.put(underUtilizedHost,vmsToMigrateFromUnderUtilizedHost);
		}
	...
	public void optimizeVmAllocationCandidates(List<PowerHost> excludedOverUtilizedHosts, List<PowerHost> excludedUnderUtilizedHosts){
		this.dstCandidateMatrix.clear();
		underUtilizedHostVMsMap.clear();
		List<PowerHostUtilizationHistory> overUtilizedHosts = getOverUtilizedHosts();

		overUtilizedHosts.removeAll(excludedOverUtilizedHosts);
		overUtilizedHosts.removeAll(excludedUnderUtilizedHosts);

		List<? extends Vm> vmsToMigrate = getVmsToMigrateFromHosts(overUtilizedHosts);
		this.dstCandidateMatrix = getVmPlacementCandidateMatrix(vmsToMigrate, new HashSet<Host>(
				overUtilizedHosts));

		//under-utilized migrations
		List<Map<String, Object>> migrationMap = new LinkedList<Map<String, Object>>();
		List<PowerHost> switchedOffHosts = getSwitchedOffHosts();
		...
		excludedHostsForFindingUnderUtilizedHost.addAll(switchedOffHosts);
		excludedHostsForFindingUnderUtilizedHost.addAll(extractHostListFromMigrationMap(migrationMap));
		excludedHostsForFindingUnderUtilizedHost.addAll(excludedUnderUtilizedHosts);
		excludedHostsForFindingUnderUtilizedHost.addAll(excludedOverUtilizedHosts);
		...
		while(true) {
			...
			if(excludedUnderUtilizedHosts.contains(underUtilizedHost)){
				continue;
			}
	...
	public void optimizeVmAllocationCandidates(Map<Host,Vm> overUtilizedVmHostMap, Map<Host, List<? extends Vm>> underUtilizedVmHostMap, List<Map<String, Object>> migrateMap){
		//clear the matrix for the new round as the resource has been updated
		//given the updated over and under utilized Hosts and corresponding Vms
		this.dstCandidateMatrix.clear();
		List<Vm> overUtilizedVmtoMigList = new ArrayList<>();
		List<PowerHostUtilizationHistory> overUtilizedHosts = new ArrayList<>();
		for(Map.Entry<Host, Vm> en:overUtilizedVmHostMap.entrySet()){
			overUtilizedVmtoMigList.add(en.getValue());
			overUtilizedHosts.add((PowerHostUtilizationHistory) en.getKey());
		}
		this.dstCandidateMatrix = getVmPlacementCandidateMatrix(overUtilizedVmtoMigList, new HashSet<Host>(
				overUtilizedHosts));
		...
		excludedHostsForFindingUnderUtilizedHost.addAll(overUtilizedHosts);
		excludedHostsForFindingUnderUtilizedHost.addAll(switchedOffHosts);
		//excludedHostsForFindingUnderUtilizedHost.addAll(extractHostListFromMigrationMap(migrateMap));
		...
		for(Map.Entry<Host,List<? extends Vm>> en:underUtilizedVmHostMap.entrySet()){
			en.getKey();
			List<? extends Vm> vmsToMigrateFromUnderUtilizedHost = en.getValue();
			this.dstCandidateMatrix.addAll(getVmPlacementCandidateMatrix(vmsToMigrateFromUnderUtilizedHost,
					excludedHostsForFindingNewVmPlacement));
		}
	}
	 ...
	 *
	 */
	public void optimizeVmAllocationCandidates(List<? extends Vm> vmList) {
		//Initial the potential vm-potential dst matrix (Mapping list)
		this.dstCandidateMatrix.clear();
		...
		while(true) {
			...
			for(Vm vm:vmsToMigrateFromUnderUtilizedHost){
				Host src = getHost(vm);
				idtoVmMap.put(vm.getId(), vm);
				if(src == null || vm.getHost()==null) {
					getVmTable().put(vm.getUid(), underUtilizedHost);
					vm.setHost(underUtilizedHost);
				}
			}
			for(Vm vm:vmsToMigrateFromUnderUtilizedHost){
				Host src = getVmTable().get(vm.getUid());
	...
	protected List<Map<String, Object>> getMigrationMapFromUnderUtilizedHosts(
		...
		while (true) {
			...
			//@tianzhangh
			//add potential destination hosts in getNewVmPlacementFromUnderUtilizedHost()
	...
	/**
	 * Gets the candidate vm placement matrix
	 *
	 * @param vmsToMigrate the vms to migrate
	 * @param excludedHosts the excluded hosts
	 * @return the vm placement matrix
	 */
	protected List<Map<Integer, List<? extends Host>>> getVmPlacementCandidateMatrix(
			List<? extends Vm> vmsToMigrate,
			Set<? extends Host> excludedHosts
	){
		List<Map<Integer, List<? extends  Host>>> candidateMatrix = new ArrayList<>();
		PowerVmList.sortByCpuUtilization(vmsToMigrate);
		for (Vm vm: vmsToMigrate){
			List<PowerHost> candidateHost = findHostCandidateForVm(vm, excludedHosts);
			if (candidateHost.size() != 0) {
				Map<Integer, List<? extends Host>> candidate = new HashMap<>();
				candidate.put(vm.getId(), candidateHost);
				candidateMatrix.add(candidate);
			}
		}
	...
	protected List<Map<String, Object>> getNewVmPlacement(
		...
		for (Vm vm : vmsToMigrate) {
			//@tianzhang
			if(this.singleVMViewSelection == true) {
				List<PowerHost> minHostList = findHostCandidateForVm(vm, excludedHosts);
				Map<Integer, List<? extends Host>> vmDstMap = new HashMap<>();
				vmDstMap.put(vm.getId(), minHostList);
				this.dstCandidateMatrix.add(vmDstMap);
			}
			...
			if (allocatedHost != null) {
				...
				//@tianzhangh
				if(hostHitPolicy) {
					int hit = this.selectedHostHits.get(allocatedHost);
					this.selectedHostHits.put(allocatedHost, hit + 1);
				}
				...
				System.out.println("PVMAPMB@getNewPlacement returned null");
				Log.printLine("@PVMAPMBF@getNewPlacemenr returned null");
				//System.exit(0);
	...
	public PowerHost findHostForVm(Vm vm) {
		...
		if (vm.getHost() != null) {  // excludes the current host for new allocation
	...
	/**
	 * Find host candidates for vm
	 * @param vm the vm
	 * @param excludedHosts the excluded hosts
	 * @return the power host lists
	 */
	public List<PowerHost> findHostCandidateForVm(Vm vm, Set <? extends Host> excludedHosts) {
		double minPower = Double.MAX_VALUE;
		List<PowerHost> minHostList = new ArrayList<>();
	...
	//@tianzhangh
	private Map<Host, Integer> selectedHostHits = new HashMap<>();
	...
	public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		...
		//@Shashi
		/*if(allocatedHost == null){
			System.out.println("PVMAPMB@FindHostForVM returned null");
			Log.printLine("@PVMAPMBF@indHostForVM returned null");
			System.exit(0);
		}*/
		...
		if(minHostList.size()!=0){
			...
			for(PowerHost dst: minHostList){
				int dstHit = 0;
				if(selectedHostHits.containsKey(dst)){
					dstHit = selectedHostHits.get(dst);
				}else{
					selectedHostHits.put(dst, 0);
				}
				if(dstHit < minHit){
					minHit = dstHit;
					allocatedHost = dst;
		...
		//processVmCreate also call this function
		//Map<Integer, List<? extends Host>> potentialDstMap = new HashMap<>();
		//potentialDstMap.put(vm.getId(),minHostList);
		//this.dstCandidateMatrix.add(potentialDstMap);
	...
	protected List<Map<String, Object>> getNewVmPlacementFromUnderUtilizedHost(
		...
		PowerVmList.sortByCpuUtilization(vmsToMigrate);
		//@tianzhang
		List<Map<Integer, List<? extends Host>>> tempvmDstMapList = new ArrayList<>();

		for (Vm vm : vmsToMigrate) {
			//@tianzhang
			if(singleVMViewSelection) {
				List<PowerHost> vmDstList = this.findHostCandidateForVm(vm, excludedHosts);
				Map<Integer, List<? extends Host>> vmDstMap = new HashMap<>();
				vmDstMap.put(vm.getId(), vmDstList);
				tempvmDstMapList.add(vmDstMap);
			}
			...
			if (allocatedHost != null) {
				...
				for (Map<String, Object> map : migrationMap) {
					...
					Host dst = (Host) map.get("host");

					//@tianzhangh
					if(hostHitPolicy) {
						int hit = this.selectedHostHits.get(dst);
						this.selectedHostHits.put(dst, hit - 1);
					}
				}
				migrationMap.clear();
				break;
		...
		//@tianzhang
		if(singleVMViewSelection)
			this.dstCandidateMatrix.addAll(tempvmDstMapList);
